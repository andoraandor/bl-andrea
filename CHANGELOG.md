...
### Added
- **Toaster Alert**: New `.toaster` component replacing old `.top-toast`. Three variants: `.toaster-info` (neutral border), `.toaster-success` (blue border), `.toaster-error` (red border). Includes title, description, optional `.btn-text` action link, and close button. 485px fixed-width, slides in from right. Dark mode auto-adapts.
- **Feedback Icons**: Three multi-color badge icons in icons.css — `.icon-fb-info` (navy square + white "i"), `.icon-fb-success` (blue square + white ✓), `.icon-fb-error` (red square + white ✗). Used by toaster alerts and other feedback components.
